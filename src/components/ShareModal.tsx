...
export default function ShareModal({ isOpen, url, onClose, onToast }: ShareModalProps) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm bg-neutral-900 border border-white/10 rounded-3xl shadow-2xl p-6 relative animate-pop-in text-right">
        <button
          onClick={onClose}
        ...
        <div className="bg-neutral-950 border border-white/5 rounded-xl p-3 text-xs text-gray-400 font-mono select-all break-all overflow-hidden text-left mb-4 select-none">
          {url}
        </div>
        ...
        <div className="flex gap-2 justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-neutral-800 text-gray-300 hover:text-white hover:bg-neutral-700 font-medium rounded-full text-xs cursor-pointer transition-colors"
          >
            إلغاء
          </button>
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 px-5 py-2 bg-white hover:bg-neutral-150 text-black font-bold rounded-full text-xs cursor-pointer transition-colors shadow-lg"
          >
