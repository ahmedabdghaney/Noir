...
export default function LiveSports({ API_KEY = 'ea2ad74f3128a87d62b665e08ae9b799' }: LiveSportsProps) {
  ...
  // Streaming source state: 'sportsrc' or 'fblive'
  const [activeSource, setActiveSource] = useState<'sportsrc' | 'fblive'>('fblive');
  // Live Broadcast Stream URL (default provided by user)
  const [streamUrl, setStreamUrl] = useState('http://194.163.159.151/fb-live/');

  // Popular Live Broadcast channel presets
  const streamPresets = [
    { name: 'قناة البث المباشر الرئيسية ⚽', url: 'http://194.163.159.151/fb-live/' }
  ];
  ...
  const currentIframeSrc = activeSource === 'sportsrc' ? embedUrl : streamUrl;
    ...
    <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-8 animate-fade-in" id="live-sports-section">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3">
            ...
            <p className="text-zinc-400 text-sm md:text-base max-w-2xl leading-relaxed font-sans">
              مرحباً بك في البث الرياضي التلفزيوني المباشر. متصل مباشرة بخدمات بث رياضية قوية مع خيارات سيرفرات <span className="text-rose-400 font-semibold font-mono">SportSRC</span> وبوابات البث المباشر الذكية <span className="text-rose-400 font-semibold font-mono">Live Broadcast</span>.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="bg-zinc-800/80 backdrop-blur-md border border-white/10 px-4 py-2.5 rounded-2xl flex items-center gap-3">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-zinc-300 font-semibold font-mono leading-none mt-1">Live Broadcast Active</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex bg-zinc-950 border border-white/5 p-1 rounded-2xl w-full max-w-xl mx-auto mb-8 shadow-inner">
        ...
        >
          ...
          البث المباشر الرئيسي (Live Stream)
        </button>
        <button
          onClick={() => {
        ...
        <div className="w-full">
          {activeSource === 'sportsrc' ? (
            ...
            <div className="space-y-2">
              <span className="text-xs font-bold text-zinc-500 block">سيرفرات وقنوات البث المباشر النشطة:</span>
              <div className="flex flex-wrap gap-2 p-1 bg-black/30 rounded-2xl border border-white/5">
        ...
        <div className="space-y-6">
          ...
          {activeSource === 'fblive' ? (
            <div className="bg-zinc-900/80 border border-white/10 rounded-2xl p-5 space-y-4 shadow-xl">
              ...
              <div className="space-y-3">
                {/* Dynamic Stream URL input */}
                <div className="space-y-1">
                  <label className="text-[11px] text-zinc-400 font-bold block">رابط البث المباشر (Stream URL)</label>
                  <div className="flex flex-col gap-2">
                    <input
                      ...
                      placeholder="http://194.163.159.151/fb-live/"
                    />
                    <button
                      onClick={handleReload}
                      className="w-full py-1.5 bg-rose-500 hover:bg-rose-600 text-white rounded-xl text-xs font-bold transition-all cursor-pointer border border-white/5"
                    >
                      تحديث البث 🔄
                    </button>
                  </div>
                  <p className="text-[10px] text-zinc-500 leading-normal mt-1">
                    يمكنك تغيير الرابط أعلاه يدوياً في أي وقت لتجربة بث مباشر مخصص أو سيرفر بديل.
                  </p>
                </div>
                ...
                <div className="pt-2 border-t border-white/5">
                  ...
                  <pre className="text-[9px] bg-black border border-white/5 p-2 rounded-xl text-emerald-400 overflow-x-auto font-mono max-h-24 select-all leading-relaxed whitespace-pre-wrap">
                    {`<iframe src="${streamUrl}" width="100%" height="380px" style="border:0;" allow="autoplay; fullscreen *; picture-in-picture *; encrypted-media" allowfullscreen></iframe>`}
                  </pre>
                </div>
              </div>
            </div>
